...
<div class="container-fluid px-md-5">
    <div class="row">
		...
		<?php
			...
			// Kiểm tra biến POST của giọng nói có tồn tại không
			if(isset($_POST["q"])){
				// Hàm urldecode giải mã chuỗi đã mã hóa bằng phương thức URL-encoded. Ký tự bị mã hóa trong chuỗi đó sẽ được giải mã trở lại thành ký tự gốc tương ứng
				$q  = urldecode($_POST["q"]);
				...
				// Tìm kiếm bằng giọng nói kết hợp lọc danh mục, phân trang
				if ($sort_asc == 'asc') {
					$timkiem = "SELECT * FROM sanpham WHERE tensp LIKE '%$q%' OR maloai IN (SELECT maloai FROM loaisanpham WHERE tenloai LIKE '%$q%') ORDER BY giaban ASC LIMIT $start, $items_per_page";
				
				} elseif ($sort == 'desc') {
					$timkiem = "SELECT * FROM sanpham WHERE tensp LIKE '%$q%' OR maloai IN (SELECT maloai FROM loaisanpham WHERE tenloai LIKE '%$q%') ORDER BY giaban DESC LIMIT $start, $items_per_page";
				
				} else {
					if ($q == '') {
						$timkiem = "SELECT * FROM sanpham WHERE giaban > 0 LIMIT $start, $items_per_page";
					} else {
						$timkiem = "SELECT * FROM sanpham WHERE tensp LIKE '%$q%' OR maloai IN (SELECT maloai FROM loaisanpham WHERE tenloai LIKE '%$q%') AND giaban > 0 LIMIT $start, $items_per_page";
					}
				}
